...
telebot.on('callback_query', (query) => {
    ...
    // Handle the callback data based on your application logic
    if (data === 'deposit') {
      // Handle button 1 callback
      telebot.sendMessage(chatId, '님 예치하기 클릭함');
    } else if (data === 'withdrawal') {
      // Handle button 2 callback
      telebot.sendMessage(chatId, '님 출금하기 클릭함');
    } else if (data === 'wallet') {
      // Handle button 2 callback
      telebot.sendMessage(chatId, '님 내지갑 클릭함');
    } else if (data === 'exchange') {
      // Handle button 2 callback
      telebot.sendMessage(chatId, '님 환전하기 클릭함');
    }
});
...
app.post(`/bot${TELEGRAM_TOKEN}`, (req, res) => {
    ...
    if (telebot.isPolling()) {
        telebot.stopPolling()
        .then(r=>console.log('3 stopPolling then r:', r))
        .catch((error) => {
            console.log("stopPolling() catch() error.code:", error.code); 
            console.log(error.response.body); 
        });
